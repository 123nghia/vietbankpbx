...
import { getUserId, isAuthenticated } from '../utils/permission.js';
...
export default function createCallRoutes(io) {
  ...
  router.post('/auto-dial', async (req, res) => {
    try {
      ...
      const result = await callManager.autoDial(
        ...
        {
          ...validated.metadata,
          requestedByUserId: getUserId(req) || 'system',
   ...
   * - status: completed|busy|no_answer|cancelled|failed
  ...
  router.get('/active/count', async (req, res) => {
    try {
      res.json(ApiResponse.success({
        activeCallsCount: callManager.getActiveCallsCount(),
  ...
  router.get('/active', async (req, res) => {
    try {
      ...
      res.json(ApiResponse.success({
        count: calls.length,
        calls
      }, 'Active calls retrieved'));
    } catch (error) {
      logger.error('Get active calls error', { error: error.message });
      res.status(500).json(ApiResponse.error('Failed to get active calls'));
    }
  });
  ...
  router.post('/:callId/end', async (req, res) => {
    try {
      ...
      const statusCode = error.statusCode || 500;
      res.status(statusCode).json(ApiResponse.error(error.message || 'Failed to end call'));
    }
  });
